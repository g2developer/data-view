 ...
 */
function DataView(params, closure){

	// closure
	this.scope = closure || window;
	...
	var dv = this;
	document.addEventListener("DOMContentLoaded", function(e){_dvinit(dv);}, false);
	document.addEventListener("DOMNodeInserted", function(e){_dvinit(dv);}, false);
...
DataView.prototype.config = function(params){
	...
	// tag attribute 이름 변경
	if(params && params.attr) ATTR_DATA_VIEW = params.attr;
	if(params && params.listAttr) ATTR_DATA_LIST = params.listAttr;
	if(params && params.noDataAttr) ATTR_NO_DATA_VIEW = params.noDataAttr;

	// dataview item들을 전역변수로 생성할 것인지
	this.createGlobalItems = (params && params.hasOwnProperty("createGlobalItems") ? params.createGlobalItems : true);
	this.referenceData = (params && params.hasOwnProperty("referenceData") ? params.referenceData : false);
...
DataView.prototype.addDataViewList = function(targetList, scope){
	...
	for(var i = 0; i < targetList.length; i++){
		...
		dvItem = new DataViewItem({
			...
			parentNode : targetList[i].parentNode,
...
DataView.prototype.getDataViewItem = function(dataViewId){
	...
	for(var i = 0; i < this.items.length; i++){
		if(this.items[i].id == dataViewId){
			...
			item.parentNode = item.elements[0].parentNode;
...
DataView.prototype.getDataViewElements = function(item){
	var e = el(item.selector, item.parentNode);
	if(!e || e.length == 0) {
		e = el(item.selector);
		if(e || e.length > 0) {
			item.parentNode = e.parentNode;
		}
	}
	return e;
};
	...
	if(!item.data || item.data.length == 0){
		...
		if(noDvElement.outerHTML){
			...
			if(item.noDataViewHTML){
				if(item.parentNode && item.parentNode.length > 0){
					item.parentNode.appendChilde(this._createElements(item.noDataViewHTML));
				}
			}
		}
...
DataView.prototype.change = function(dataViewId, data){
	...
	// !! parentNode로 찾으면 parentNdoe를 변경되는 ui 프레임워크에 의해 못찾을 수 있음
	if(html && html != ""){
...
/**
 * dataview의 parentNode를 다시 찾아서 등록한다.
 * dataview가 화면에 로드되기 전 dataview의 parentNode의 경우 화면의 객체가 아니기 때문에..
	...
	for(var i = 0; i < targetList.length; i++){
		...
		for(var j = 0; j < this.items.length; j++){
			...
			if(this.items[j].id == dvId){
				this.items[j].parentNode = el(targetList[i]).parentNode;
			}
		}
	}
};
		...
		set : function(d){
			...
			if(typeof(obj["_"+prop]) == "object" && bindChildArr && bindChildArr.length > 0){
				if(Array.isArray(obj["_"+prop])){
					obj["_"+prop] = dv.bindList(obj["_"+prop], bindChildArr, obj);
				}else {
					dv.bind(obj["_"+prop], bindChildArr, obj);
				}
			}
...
DataView.prototype.bindList = function(arr, bindArr, listener){
	...
	for(var i = 0; i < arr.length; i++){
		if(isObj(arr[i])){
			this.bind(arr[i], arr.bindArr, arr);
		}
	...
	arr.push = function(){
		...
		if(isObj(item)){
			this.bind(item, this.bindArr, this);
	...
	if(Proxy){
			...
			set : function(target, index, value, receiver){
				...
				if(!isNaN(index)){
					if(typeof(value) == "object"){
						dv.bind(value, target.bindArr, target);
					}
		...
		for(var k in this.scope){
			if(["webkitStorageInfo"].indexOf(k) == -1 && Array.isArray(this.scope[k]) && this.scope[k] == arr){
				//eval(k + " = parr;");
				this.scope[k] = parr;
				break;
			}
	...
	set data(obj){
		if(this.isBind){
			...
			if(this.type == ATTR_DATA_VIEW){
				...
				this.parent.bind(obj, this.normalVarKeys, this);
				this._data = obj;
				...
				var parr = this.parent.bindList(obj, this.normalVarKeys, this);
				this._data = parr;
			}
			this.refresh();
		} else {
...
DataViewItem.prototype.change = function(data){
	...
	this.parent.change(this.id, data);
};
...
DataViewItem.prototype.refresh = function(refreshData){
	...
	if(!refreshData || this.type == ATTR_DATA_VIEW){
		this.parent.refresh(this.id, refreshData);
		return;
	}
...
DataViewItem.prototype.append = function(data){
	this.parent.append(this.id, data);
};
...
DataViewItem.prototype.prepend = function(data){
	this.parent.prepend(this.id, data);
};
...
DataViewItem.prototype.update = function(idx, data){
	this.parent.update(this.id, idx, data);
};
...
DataViewItem.prototype.remove = function(idx){
	this.parent.remove(this.id, idx);
};
...
DataViewItem.prototype.objToForm = function(obj){
	this.parent.objToForm(this.id, obj);
};
...
DataViewItem.prototype.formToObj = function(){
	return this.parent.formToObj(this.id);
};
...
DataViewItem.prototype.form = function(data){
	return this.parent.form(this.id, data);
};
